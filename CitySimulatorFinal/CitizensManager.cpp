...
int CitizensManager::getTotalPopulation() const {
    return HighClassCitizen::getHighClassCount() +
           MidClassCitizen::getMidClassCount() +
           LowClassCitizen::getLowClassCount();
}

int CitizensManager::getTotalEmployed() const {
    std::vector<PrototypeCitizen*> rawCitizens;
    rawCitizens.reserve(citizens.size());
    for (const auto& citizen : citizens) {
        rawCitizens.push_back(citizen.get());
    }
    return CitizenStatistics::totalEmployed(rawCitizens);
}
